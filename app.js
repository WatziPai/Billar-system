...
function actualizarConsumoDueno() {
    ...
    if (consumosActuales.length === 0) {
        console.log('⚠️ Mostrando mensaje de sin consumos');
        ...
        console.log('✅ HTML de mensaje vacío insertado');
        return;
    }
    
    // Si hay consumos, mostrar la lista completa
    ...
    let html = '<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">' +
        ...
        '<div><strong style="font-size: 16px; color: #856404;">📊 Total Consumido (No Cobrado)</strong>' +
    ...
    consumosOrdenados.forEach(c => {
        html += '<div style="background: white; border: 1px solid #e0e0e0; border-radius: 8px; padding: 15px; margin-bottom: 12px; border-left: 4px solid #ff9800;">' +
            ...
            '<div style="flex: 1;"><div style="font-size: 13px; color: #999; margin-bottom: 8px;">🍽️ ' + c.fecha + '</div>' +
            '<div style="background: #fff3cd; padding: 10px; border-radius: 6px;">' +
            '<strong style="font-size: 12px; color: #856404;">Productos:</strong><div style="margin-top: 8px;">';
        
        c.productos.forEach(p => {
            html += '<div style="font-size: 12px; color: #856404; padding: 3px 0; display: flex; justify-content: space-between;">' +
                '<span>• ' + p.nombre + ' x' + p.cantidad + '</span>' +
                '<span><strong>S/ ' + (p.precio * p.cantidad).toFixed(2) + '</strong></span></div>';
        });
        
        html += '</div></div></div>' +
            '<div style="margin-left: 20px;"><div style="font-size: 22px; font-weight: bold; color: #ff9800;">S/ ' + c.total.toFixed(2) + '</div></div></div>';
